    ...
    <div className='rounded-md border border-white/10 bg-white/6 p-5'>
      <div className='space-y-2'>
        ...
        <div className='text-xs text-white/45'>
          Начислено:{' '}
          <span className='text-(--color-brand-home)'>
            {formatPoints(
        ...
        <div className='text-xs text-white/45'>
          EXP:{' '}
          <span className={expDelta >= 0 ? 'text-emerald-300' : 'text-rose-400'}>
            {expDelta >= 0 ? `+${expDelta}` : expDelta}
          </span>
        </div>
        <div className='text-xs text-white/45'>
        ...
        <div className='text-xs text-white/45'>Дедлайн: {formatRelativeDate(task.deadlineAt)}</div>
        <div className='text-xs text-white/35'>Создано: {formatRelativeDate(task.createdAt)}</div>
      </div>
    </div>
  )
}
