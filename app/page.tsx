...
type ModalKey = 'household' | 'shopping-list' | 'shopping-actions' | 'shopping-replace' | null
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(250,204,21,0.12),_transparent_20%),linear-gradient(180deg,_#f3efe3_0%,_#ebe7d8_45%,_#dde7df_100%)] text-slate-950">
      {modal ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/55 p-3 backdrop-blur-md sm:items-center sm:p-6">
                    ...
                    {sortedShoppingItems.map((item) => (
                      ...
                      >
                        <div className="space-y-2">
                          ...
                          {item.note ? <p className="text-sm leading-6 text-white/70">{item.note}</p> : null}
                          <div className="text-xs text-white/45">
          ...
          {modal === 'shopping-actions' && selectedShoppingItem ? (
            ...
            >
              ...
              <div className="mt-5 space-y-3">
                ...
                >
                  {busyKey === `product-${selectedShoppingItem.id}` ? 'Обновляю...' : 'Отметить купленным'}
                </button>
          ...
          {modal === 'shopping-replace' && selectedShoppingItem ? (
            ...
            >
              <div className="space-y-2">
                <div className="text-xs uppercase tracking-[0.3em] text-white/50">Заменить позицию</div>
              ...
              <div className="mt-5 space-y-3">
                ...
                >
                  {busyKey === `replace-${selectedShoppingItem.id}` ? 'Сохраняю...' : 'Сохранить замену'}
                </button>
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-4 px-4 py-4 sm:px-6 sm:py-6">
          ...
          <div className="space-y-6 px-5 py-6 sm:px-8 sm:py-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="rounded-[1.5rem] border border-white/10 bg-white/8 px-4 py-4">
                <div className="text-xs uppercase tracking-[0.24em] text-white/50">Сейчас в Mini App</div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          >
            <div className="space-y-2">
              <div className="text-xs uppercase tracking-[0.28em] text-slate-500">Новая бытовая задача</div>
          ...
          >
            <div className="space-y-2">
              <div className="text-xs uppercase tracking-[0.28em] text-slate-500">Новая покупка</div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          >
            ...
            {loading ? (
              <div className="mt-5 rounded-[1.4rem] border border-dashed border-black/10 bg-[#f8f5ec] px-4 py-10 text-center text-slate-500">
                Загружаю...
              </div>
            ) : sortedTasks.length ? (
              <div className="mt-5 space-y-3">
                {sortedTasks.slice(0, 3).map((task) => (
                  <article
                    key={task.id}
                    className="rounded-[1.4rem] border border-black/8 bg-[#f8f5ec] p-4"
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button
                        type="button"
                        className="rounded-full bg-[#f3c54b] px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-[#eabf36] disabled:opacity-60"
                        onClick={() => void completeTask(task)}
                        disabled={busyKey === `task-${task.id}`}
                      >
                        {busyKey === `task-${task.id}` ? '...' : 'Готово'}
                      </button>
                    </div>
                  </article>
                ))}
          ...
          >
            ...
            {loading ? (
              <div className="mt-5 rounded-[1.4rem] border border-dashed border-black/10 bg-[#eef7f0] px-4 py-10 text-center text-slate-500">
                Загружаю...
              </div>
            ) : sortedShoppingItems.length ? (
              <div className="mt-5 space-y-3">
                {sortedShoppingItems.slice(0, 3).map((item) => (
                  <button
                    key={item.id}
                    type="button"
                    className="w-full rounded-[1.4rem] border border-black/8 bg-[#eef7f0] p-4 text-left transition hover:border-slate-400"
                    onClick={() => openShoppingActions(item)}
                  >
                    <div className="space-y-2">
                      <span
