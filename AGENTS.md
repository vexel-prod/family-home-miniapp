...
## Frontend / Layout quality bar
При любых изменениях UI считай задачу незавершенной, если есть хотя бы один из дефектов:
- секции наезжают друг на друга;
- текст выходит за границы контейнера;
- кнопки, инпуты, карточки или модалки вылезают за viewport;
- появляются горизонтальный скролл или обрезание важного контента;
- элементы ломаются на узких экранах;
- заголовки/подзаголовки теряют читаемую иерархию;
- spacing между секциями нестабилен;
- длинные строки, бейджи, ссылки, кодовые фрагменты или значения не переносятся корректно.

## Обязательные проверки после изменений UI
После каждой правки интерфейса:
1. проверь затронутые экраны на mobile, tablet и desktop;
2. проверь состояния с длинным текстом, пустыми данными и ошибками;
3. проверь, нет ли overflow по ширине и высоте;
4. проверь, что layout не разваливается при 125% и 150% zoom;
5. проверь hover/focus/disabled/loading состояния;
6. запусти имеющиеся тесты и линтер;
7. в отчете явно напиши, какие layout-риски проверены.

## Правила реализации
- Сначала короткий план.
- Сначала ищи существующие layout primitives и design tokens, а не изобретай новые.
- Не используй магические отступы и случайные фиксированные высоты без причины.
- Предпочитай устойчивые layout-паттерны: flex/grid, minmax, wrap, clamp, overflow handling.
- Для текста всегда продумывай поведение при длинных строках.
- Если компонент потенциально хрупкий, добавь или обнови тест/стори/пример.

## Done when
Задача завершена только если:
- код собран;
- линтер/тесты пройдены;
- нет явных layout overflow/overlap проблем;
- в финальном ответе перечислены измененные файлы и проведенные layout-проверки.
