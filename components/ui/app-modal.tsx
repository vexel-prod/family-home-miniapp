...
export function ModalOverlay({ children }: AppModalProps) {
  return (
    <div className='absolute inset-0 z-50 flex flex-col justify-center bg-(--overlay-backdrop) px-4 backdrop-blur-md'>
      {children}
    </div>
  )
}
...
export function ModalPanel({ children, wide = false, tall = false }: ModalPanelProps) {
  const classNames = [
    'w-full rounded-md border border-white/10 bg-[var(--color-panel)] text-[var(--color-panel-text)] shadow-[var(--shadow-panel)] backdrop-blur-xl',
