...
Telegram Mini App для семьи: общие домашние задачи, список покупок, месячный рейтинг и личный прогресс участника.

Это уже не приложение для двух фиксированных пользователей. Теперь это многопользовательская household-модель с onboarding, invite-кодами и ролью главы семьи.
...
Приложение работает внутри Telegram Mini App. Авторизация опирается только на валидированный Telegram `user.id`.

## 2. Как устроена модель доступа

### Telegram auth

Пользователь открывает приложение из Telegram. Сервер валидирует `initData` и получает настоящий Telegram `user.id`.
...
1. `onboarding`
   Пользователь еще не состоит ни в одной активной семье.

2. `active`
   У пользователя есть активный `Member`, привязанный к конкретному `Household`.
...
Важно: уровень и прогресс живут на уровне membership, а не глобального пользователя. Это значит, что новая семья = новый чистый прогресс.
...
Если пользователь не состоит в семье, главный экран не показывается. Вместо него открывается onboarding с двумя действиями:
...
Рейтинг сейчас считается на клиенте на основе `monthlyCompletedTasks`, который приходит из `/api/bootstrap`.
...
- [app/api/household/members/[memberId]/route.ts](/Users/vladimirpaskin/Developer/NEXTJS-projects/family-home-miniapp/app/api/household/members/[memberId]/route.ts)
  Удаление участника.
- [features/home/components/household-onboarding.tsx](/Users/vladimirpaskin/Developer/NEXTJS-projects/family-home-miniapp/features/home/components/household-onboarding.tsx)
  Onboarding UI.
- [features/home/components/household-profile-modal.tsx](/Users/vladimirpaskin/Developer/NEXTJS-projects/family-home-miniapp/features/home/components/household-profile-modal.tsx)
  Личный кабинет и семейные действия.
...
Потому что клиенту нельзя доверять household access. Семья определяется только на сервере через валидированный Telegram auth.
...
Потому что пустая семья без участников в этом продукте не имеет ценности и только создает мусор в базе.
